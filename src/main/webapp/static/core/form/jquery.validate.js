...
;(function(global, undefined){
  ...
  Validate.prototype = {
    ...
    init: function () {
      ...
      this.messager.wrap.css('z-index', this.options.zIndex + 2);
    ...
    showError: function (carry, error) {
      ...
      carry.label.on('mouseenter.validate-error', function(e) {
        that.messager.show(carry, error);
      });
      carry.flag.on('mouseenter.validate-error', function(e) {
        that.messager.show(carry, error);
      });
      ...
      carry.flag.css({
        'top': pos.top + 'px',
        'left': (pos.left + carry.label.width()) + 'px'
      });

      return this;
    },
    closeError: function (carry, success) {
      if (carry.flag) {
  ...
  methods.multiple = {
    ...
    fire: function ($ele, args, defer, form) {
      var left, right, isSelect, isCheckbox, test, length = 0,
